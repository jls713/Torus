...
void GenPar::findNN() {
  short i, n2_min = 0, n2_max = 0, n3_min = 0, n3_max = 0, n1_max = 0;
  for (i = 0; i < ntot; i++) {
    if (N1[i] > n1_max) n1_max = N1[i];
    if (N2[i] > n2_max) n2_max = N2[i];
    if (N2[i] < n2_min) n2_min = N2[i];
    if (N3[i] > n3_max) n3_max = N3[i];
    if (N3[i] < n3_min) n3_min = N3[i];
  }
  nn1 = n1_max + 1;
  nn2 = (n2_max - n2_min) / 2 + 1;
  nn3 = (n3_max - n3_min) / 2 + 1;
}
//////////////////////////////////////////////////////////////////////////////
void GenPar::sortSn()
// sort in order of n (order by n3, then by n2, then n1 if needed)
{
  if (ntot > 0) {
    register short i, ascending = 1;
    register int* M = new int[ntot];
    for (i = 0; i < ntot; i++) {
      M[i] = 1000 * (1000 * N3[i] + N2[i]) + N1[i];
...
// tk
{
  if (ntot > 0) {
    ...
    for (int i = 0; i < ntot; i++) {
      Mt = 1000 * (1000 * N3[i] + N2[i]) + N1[i];
      if (i && Mt < oldM) {
        sortSn();
        i = 0;
      }  // if not sorted, sort and restart.
      if ((i && Mt == oldM) || (Mt >= 0 && !(Mt % 1000)) ||
          N1[i] < 0) {  // if illegal n
        short* N1new = new short[--ntot];
        short* N2new = new short[ntot];
        double* Snew = new double[ntot];
...
void GenPar::MakeGeneric()
{
  ...
  unsigned count=0;
  for(int i=-nn3+1;i<nn3;i+=2)
  for(int j=-nn2+1;j<nn2;j+=2)
  for(int k=0;k<nn1;++k){
    if(!(i==0 and j==0 and k==0)){
      N3[count] = i; N2[count] = j; N1[count] = k;
      count++;
    }
  }
...
int GenPar::same_terms_as(const GenPar& G) const {
  ...
  if (nn3 != G.nn3) return 0;
  for (register short i = 0; i < ntot; i++)
    if (N1[i] != G.N1[i] || N2[i] != G.N2[i] || N3[i] != G.N3[i]) return 0;
  return 1;
}
////////////////////////////////////////////////////////////////////////////////
...
void GenPar::write_log(ostream& to) const {
  register int i, ils, n3act = 1000, n2act = -1000, n1act = 1, n1max = 0;
  to << ntot;
  // n2act = N2[0];
  for (i = 0; i < ntot; i++){
    if(N2[i]<n2act){
      to << "\nn2/n1 ";
      for(int j=i-N1[i-1]-1;j<i;j++) to << N1[j] << "  ";
      to << "\n";
    }
    if(N3[i]!=n3act) { to << "\nn3 = " << N3[i] ; n3act=N3[i];}
    if(N2[i]!=n2act){
      to<<"\n";
      if(N2[i]<-9) to<<" ";
      else if(N2[i]<0 or N2[i]>10) to<<"  ";
      else if(N2[i]<10) to<<"   ";
      to << N2[i] << " ";n2act=N2[i];
    }
    if(n3act==0 and n2act==0 and N1[i]==1) to <<" - ";
    ils = (S[i] == 0.) ? 0 : int(-10. * log10(fabs(S[i]))); n1act = N1[i];
...
void GenPar::read(istream& from) {
  ...
  register short i;
  for (i = 0; i < ntot; i++) {
    from >> N1[i] >> N2[i] >> N3[i] >> S[i];
    if (N2[i] % 2 != 0) TorusError("GenPar: odd n2 in read()", -errGen_);
    if (N3[i] % 2 != 0) TorusError("GenPar: odd n3 in read()", -errGen_);
    if (N1[i] == 0 && (N2[i] >= 0 || N3[i] >= 0))
      TorusError("GenPar: +ve n2 or n3 with n1==0 in read()", -errGen_);
  }
  sortSn();
  // SanityCheck();
  // findNN();
...
void GenPar::read(int* N1in, int* N2in, int* N3in, double* Snin, short newtot) {
  if (newtot <= 0) TorusError("GenPar: number of terms <= 0", -errGen_);
  if (newtot != ntot) {
    if (ntot > 0) {
      delete[] N1;
